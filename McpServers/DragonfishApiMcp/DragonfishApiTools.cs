...
public sealed class DragonfishApiTools(HttpClient http, IOptions<DragonfishApiConfig> cfg, SwaggerCatalog swagger)
{
    ...
    public async Task<string> ListarEntidades(
        [Description("Nombre del perfil (ver listar_perfiles), ej. \"TEST\"")] string perfil,
        [Description("Texto opcional para filtrar (ej. \"factura\")")] string? filtro = null)
    {
        var p = ResolverPerfil(perfil);
        var doc = await swagger.GetAsync(p.BaseUrl);
        var paths = doc["paths"]?.AsObject() ?? throw new InvalidOperationException("El swagger no tiene 'paths'.");
    ...
    public async Task<string> DescribirEntidad(
        ...
        [Description("Nombre de la entidad sin barras, ej. \"Factura\"")] string entidad)
    {
        ...
        var operaciones = doc["paths"]?[path]
            ?? throw new InvalidOperationException($"No existe la entidad '{entidad}' (esperaba el path '{path}'). Usá listar_entidades para ver los nombres válidos.");
    ...
    public async Task<string> Consultar(
        ...
        [Description("Query params opcionales, ej. {\"query\":\"R016\",\"limit\":\"5\"}")] Dictionary<string, string>? filtros = null)
    {
        var p = ResolverPerfil(perfil);
        var path = SwaggerCatalog.PathParaEntidad(entidad);
    ...
    public async Task<string> Crear(
        [Description("Nombre del perfil (ver listar_perfiles)")] string perfil,
        [Description("Nombre de la entidad sin barras, ej. \"Factura\"")] string entidad,
        [Description("Body en JSON crudo, según el esquema de describir_entidad")] string bodyJson)
    {
        var p = ResolverPerfil(perfil);
        var path = SwaggerCatalog.PathParaEntidad(entidad);
        var url = $"{p.BaseUrl.TrimEnd('/')}{path}";
    ...
    private DragonfishPerfil ResolverPerfil(string perfil) =>
        cfg.Value.Perfiles.TryGetValue(perfil, out var p)
            ? p
            : throw new InvalidOperationException($"No existe el perfil '{perfil}'. Usá listar_perfiles para ver los configurados.");
